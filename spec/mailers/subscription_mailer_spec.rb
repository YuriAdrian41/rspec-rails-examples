...
RSpec.describe SubscriptionMailer, :type => :mailer do
  ...
  describe ".send_confirmation_request!(subscription)" do
    ...
    it "calls the error-sensitive deliver_now!" do
      delivery = double
      expect(delivery).to_receive(:deliver_now!).with(no_args)

      expect(SubscriptionMailer).to receive(:confirmation_request).with(@subscription).and_return(delivery)
