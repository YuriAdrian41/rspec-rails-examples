...
RSpec.describe HeadlineScraperJob, :type => :job do
  ...
  it "emails headlines scraped from given URL" do
    ...
    url_to_escape = "https://eliotsykes.github.io/rspec-rails-examples/"
    recipient_email = "anchor@nightlynews.tld"

    assert_performed_with(
      job: HeadlineScraperJob,
      args: [{url: url_to_escape, recipient: recipient_email}],
      queue: 'default'
    ) do

      VCR.use_cassette("news_page") do
        HeadlineScraperJob.perform_later url: url_to_escape, recipient: recipient_email
      end
