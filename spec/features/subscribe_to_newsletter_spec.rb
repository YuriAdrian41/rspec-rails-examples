...
feature "Subscribe to newsletter" do
  ...
  context "in browser with native data input", driver: driver_with(native_data_input: true) do
  ...
  context "in browser without native date input", driver: driver_with(native_data_input: false) do
    ...
    scenario "subscribes user to newsletter" do
      ...
      expect do
        ...
        visit_emailed_confirm_subscription_link("buddy@example.tld")
        expect(page).to be_confirm_subscription_page(Subscription.last).with_subscription_starting_on("January 31st, 2015")
  ...
  def visit_new_subscription
    ...
    # 
    # Capybara's methods account that some browser operations happen asynchronously. Capybara will retry most operations for a few seconds before failing a test (Capybara.default_wait_time is 2 seconds by default). `expect ... eq ...` will not retry, whereas `expect ... have title ...` does retry. Once the new title test is passed, we can be confident that the browser's current path has updated to the new path.
